...
namespace cm
{
	namespace detail
	{
		...
		{
			CM_FUNC_QUALIFIER CM_CONSTEXPR static vec<L, T, Q> call(vec<L, T, Q> const& a, vec<L, T, Q> const& b)
			{
		...
		struct compute_vec_equal<L, T, Q, IsInt, Size, false>
		{
			CM_FUNC_QUALIFIER CM_CONSTEXPR static vec<L, T, Q> call(vec<L, T, Q> const& a, vec<L, T, Q> const& b)
			{
				bool b = true;
				for (length_t i = 0; i < L; ++i)
					b = b && detail::compute_equal<T, std::numeric_limits<T>::is_iec559>::call(a[i], b[i]);
				return b;
			}
		};
		...
		{
			CM_FUNC_QUALIFIER CM_CONSTEXPR static vec<L, T, Q> call(vec<L, T, Q> const& a)
			{
				vec<L, T, Q> v(a);
				for (length_t i = 0; i < L; ++i)
					v[i] ~= v[i];
